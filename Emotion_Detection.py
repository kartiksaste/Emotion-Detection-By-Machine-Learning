...
if model_file:
    ...
    # Initialize a session state variable for the video stream
    if 'frame' not in st.session_state:
        st.session_state.frame = None

    # Capture live video feed
    frame = st.camera_input("Capture live webcam feed")

    if frame is not None:
        # Update session state with the latest captured frame
        st.session_state.frame = frame

    # If a frame is available, process it
    if st.session_state.frame is not None:
        # Convert the image to an array
        image = np.array(bytearray(st.session_state.frame), dtype=np.uint8)
